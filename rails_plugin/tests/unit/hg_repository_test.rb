...
class HgRepositoryTest < Test::Unit::TestCase
  ...
  def test_changeset_raises_error_when_changeset_does_not_exist
    ...
    begin
      ...
      assert e.message.index("unknown revision 'bogus'")
    end
  end
  ...
  def test_changeset_returns_changeset_when_it_exists
    ...
    assert_equal 'fef857204a0c58caefe249dda038316e856e896d', changeset.changeset_identifier
    assert_equal 2, changeset.revision_number
    assert_equal "Bryan O'Sullivan", changeset.person
    assert_equal "Introduce a typo into hello.c.", changeset.desc
    assert_equal 'Sat Aug 16 20:05:04 UTC 2008', changeset.time.utc.to_s
  end
  ...
  def test_will_only_pull_once
    hg_client = HgClient.new(nil)
    def hg_client.pull
      @pull_count ||= 0
      @pull_count += 1
    end
    def hg_client.pull_count
      @pull_count
    end
    repos = HgRepository.new(hg_client, nil)
    repos.pull
    repos.pull
    assert_equal 1, hg_client.pull_count
  end
