...
class HgRepositoryCloneTest < Test::Unit::TestCase
  ...
  def setup
    ...
    @clone_dir = "#{TEST_TMP_DIR}/hg_repository_clone_test_clones/#{ActiveSupport::SecureRandom.hex(32)}"
    @clone = HgRepositoryClone.new(@repository, @clone_dir, Project.new)
  end
  ...
  def test_cannot_construct_if_error_file_exists
    ...
    begin
      HgRepositoryClone.new(@repository, @clone_dir, Project.new)
  ...
  def test_can_construct_if_error_file_exists_when_retry_specified
    ...
    HgRepositoryClone.new(@repository, @clone_dir, Project.new, true)
  end
